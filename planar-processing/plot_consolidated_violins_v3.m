...
for dataset_index = 1:num_datasets
    ...
    for window_resolution_index = 1:num_window_resolution
        ...
        results{window_resolution_index} = load(fullfile(current_read_directory, 'error_uncertainty_statistics_02.mat'));
    end
    % =============================
    ...
    for violin_index = 1:num_total_methods
        ...
        elseif violin_index <= num_uncertainty_methods
            violins{violin_index} = violinplot_single_asymm(results{1}.sigma_all_valid{violin_index}', results{2}.sigma_all_valid{violin_index}', violin_index, 0.3, colors(1, :));

            plot([violin_index, violin_index], [0, max_error_threshold], 'color', [0, 0, 0, 0.5], 'linewidth', 0.5);
            y1 = results{1}.sigma_rms(violin_index);
            y2 = results{2}.sigma_rms(violin_index);
        ...
        color_all{violin_index} = violins{violin_index}(1).FaceColor;
        ...
        % plot rms error
        plot([min(x), violin_index], results{1}.err_rms*[1, 1], 'color', [0, 0, 0, violins{violin_index}(1).FaceAlpha+0.2], 'linewidth', 2)
        plot([violin_index, max(x)], results{2}.err_rms*[1, 1], 'color', [0, 0, 0, violins{violin_index}(2).FaceAlpha+0.2], 'linewidth', 2)
    end
...
xl = set(gca, 'xticklabel', {'IM'; 'MC'; 'CS'; 'Error'; 'Unwt'; 'PD-Var'; 'Entropy'});
% xl = set(gca, 'xticklabel', [uncertainty_method_array', combination_method_names']);
...
figure_write_directory = fullfile(top_write_directory, 'consolidated-figures', ...
    ['trials=' num2str(num_trials, '%d')], ['ppr' num2str(percentage_particles_remove, '%.2f') '_nrs' num2str(num_resampling_trials) '-prewindow'], ...
    ['max_error=' num2str(max_error_threshold, '%.2f') 'pix-new-02']);
mkdir_c(figure_write_directory);
if save_figures
    save_figure_to_png_svg_fig(figure_write_directory, 'error-uncertainty-histograms-violin-all-datasets', [1, 0, 0]);
    % export_fig(fullfile(figure_write_directory, 'error-uncertainty-histograms-violin-all-datasets.png'), '-r600'); 
else
    pause(0.1);
end