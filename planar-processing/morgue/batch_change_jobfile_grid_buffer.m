    ...
    for dataset_index = 1:num_datasets
        ...
        for field_name_index = 1:numel(field_names)
            ...
            if contains(field_name, 'PIV') && ~contains(field_name, 'run') && ~contains(field_name, 'error')
                ...
                str = strsplit(Data.(field_name).winres, ',');
                window_resolution = str2double(str{1});
                % calculate corresponding gridbuffer
                grid_buffer = window_resolution/2;
                % modify grid buffer field
                Data.(field_name).gridbuf = [num2str(grid_buffer) ',' num2str(grid_buffer)];
            end
        end
