...
for window_resolution_index = 1:num_window_resolution
    ...
    for dataset_index = 1:num_datasets
        ...
        % initialize 
        % --------------------------            
        unc_ratio_iqr = cell(1, num_resampling_methods);
        unc_ratio_iqr_fit = cell(1, num_resampling_methods);
        unc_ratio_iqr_rate = cell(1, num_resampling_methods);
        ...
        for resampling_method_index = 1:num_resampling_methods            
            ...
            % initialize 
            % --------------------------            
            unc_ratio_iqr{resampling_method_index}.x = nans(num_trials, num_individual_methods, num_ppr);
            unc_ratio_iqr{resampling_method_index}.y = nans(num_trials, num_individual_methods, num_ppr);
            
            unc_ratio_iqr_fit{resampling_method_index}.x = nans(num_trials, num_individual_methods, num_ppr);
            unc_ratio_iqr_fit{resampling_method_index}.y = nans(num_trials, num_individual_methods, num_ppr);
            
            unc_ratio_iqr_rate{resampling_method_index}.x = nans(num_trials, num_individual_methods);
            unc_ratio_iqr_rate{resampling_method_index}.y = nans(num_trials, num_individual_methods);
            ...
            for trial_index = 1:num_trials
                ...
                for individual_method_index = 1:num_individual_methods
                    ...
                    unc_ratio_iqr{resampling_method_index}.x(trial_index, individual_method_index, :) = iqr(unc_ratio_x, 1);
                    unc_ratio_iqr{resampling_method_index}.y(trial_index, individual_method_index, :) = iqr(unc_ratio_y, 1);

                    % --------------------------
                    % calculate rate of change
                    % --------------------------
                    % extract values
                    fx = unc_ratio_iqr{resampling_method_index}.x(trial_index, individual_method_index, :);
                    fy = unc_ratio_iqr{resampling_method_index}.y(trial_index, individual_method_index, :);
                    ...
                    % extract fit values
                    unc_ratio_iqr_fit{resampling_method_index}.x(trial_index, individual_method_index, :) = fitobject_x(percentage_particles_remove_array);
                    unc_ratio_iqr_fit{resampling_method_index}.y(trial_index, individual_method_index, :) = fitobject_y(percentage_particles_remove_array);

                    % extract slope
                    unc_ratio_iqr_rate{resampling_method_index}.x(trial_index, individual_method_index) = fitobject_x.p1;
                    unc_ratio_iqr_rate{resampling_method_index}.y(trial_index, individual_method_index) = fitobject_y.p1;
                end

                % calculate weights
                wt_x = 1 ./ abs(unc_ratio_iqr_rate{resampling_method_index}.x(trial_index, :));
                wt_y = 1 ./ abs(unc_ratio_iqr_rate{resampling_method_index}.y(trial_index, :));
        ...
        current_read_directory = fullfile(top_write_directory, dataset_name, ['WS' num2str(window_resolution_index)], 'weights-rms-change-study');
        % file name
        filename = ['resampling-statistics-nt=' num2str(num_trials, '%d') 'nr='  num2str(num_resampling_trials, '%d') '-weights-unc-iqr.mat'];
        % load results to file
        save(fullfile(current_read_directory, filename), 'unc_ratio_iqr', 'unc_ratio_iqr_fit', 'pdf_w', ...
