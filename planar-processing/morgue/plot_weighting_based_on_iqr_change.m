...
for window_resolution_index = 1:num_window_resolution
    ...
    for dataset_index = 1:num_datasets
        ...
        if save_figures
            figure_save_directory = fullfile(current_read_directory, 'figures-iqr');
            mkdir_c(figure_save_directory);
        end
                
        % % ==========================
        % % plot variation of iqr with particle % for just one trial
        % % ==========================
        % trial_index = 100;
        % ppr = percentage_particles_remove_array*100;

        % plot_unc_ratio_with_particle_removal(unc_ratio_iqr, unc_ratio_iqr_fit, trial_index, ppr, ...
        %                                     individual_method_array, resampling_method_names_plot, 'Uncertainty Ratio IQR', user_screen_resolution);

        % if save_figures
        %     save_figure_to_png_svg_fig(figure_save_directory, 'iqr-rate', [1, 0, 0])
        % end
        ...
        % ==========================
        unc_indiv_mag = [unc_indiv.x; unc_indiv.y];
        unc_comb_mag = [unc_comb.x; unc_comb.y];
        unc_indiv_2 = mat2cell(unc_indiv_mag, num_trials*2, ones(1, num_individual_methods));
        unc_comb_2 = mat2cell(unc_comb_mag, num_trials*2, ones(1, num_resampling_methods));
