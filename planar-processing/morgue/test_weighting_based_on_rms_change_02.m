...
for window_resolution_index = 1:num_window_resolution
    ...
    for dataset_index = 1:num_datasets
        ...
        if save_figures
            figure_save_directory = fullfile(current_write_directory, 'figures-2');
        ...
        for resampling_method_index = 1:num_resampling_methods
            ...
            wt_trials{resampling_method_index} = nans(num_trials, num_individual_methods);
            % ================================================
            %% loop through trials
            % ================================================
            for trial_index = 1:num_trials
                ...
                wt_trials{resampling_method_index}(trial_index, :) = wt ./ sum(wt);
            end            
        end
        ...
        err_avg = mean(err_U_trials, 'omitnan');
        err_abs_avg = mean(abs(err_U_trials), 'omitnan');
        % err_rms = std(err_U_trials, [], 'omitnan');
        % calculate confidence interval for the error
        confint_err = (prctile(err_U_trials, 84) - prctile(err_U_trials, 16))/2;
        confint_err_abs = (prctile(abs(err_U_trials), 84) - prctile(abs(err_U_trials), 16))/2;
        ...
        for resampling_method_index = 1:num_resampling_methods
            for individual_method_index = 1:num_individual_methods
                pdf_w{resampling_method_index, individual_method_index} = histcounts(wt_trials{resampling_method_index}(:, individual_method_index), bins_w, 'normalization', 'pdf');
            end
        end
        ...
        % ==========================
        unc_comb = nans(num_trials, num_resampling_methods);
        sigma_rms_comb = nans(1, num_resampling_methods);
        for resampling_method_index = 1:num_resampling_methods
            unc_comb(:, resampling_method_index) = sum(unc_indiv  .* wt_trials{resampling_method_index}, 2); 
            sigma_rms_comb(resampling_method_index) = rms(unc_comb(:, resampling_method_index), 'omitnan');
        end
        ...
        for method_type = [1, 2]
            ...
            for method_index = 1:num_methods
                for model_index = 1:num_models
                    ...
                    d = calculate_histogram_distance(abs(err_U_trials), abs(y), bins, histogram_distance_method);
        ...
        % display figures
        if display_figures
            compare_error_models(error_models, err_U_trials, err_est_indiv, err_est_comb, individual_method_array, resampling_method_names_plot_short, ...
        ...
        if display_figures
            ...
            % ==========================
            unc_indiv_2 = mat2cell(unc_indiv, num_trials, ones(1, num_individual_methods));
            unc_comb_2 = mat2cell(unc_comb, num_trials, ones(1, num_resampling_methods));
            % violins = make_violin_plot_error_uncertainty(abs(err_U_trials - mean(err_U_trials, 'omitnan')), unc_indiv_2, unc_comb_2, ...
            %                                                 err_rms, sigma_rms_indv, sigma_rms_comb, ...
            %                                                 individual_method_array, resampling_method_names_plot_short, ...
            %                                                 colors, user_screen_resolution, max_error_threshold);
            violins = make_violin_plot_error_uncertainty_02(abs(err_U_trials), unc_indiv_2, unc_comb_2, ...
                                                            err_rms, sigma_rms_indv, sigma_rms_comb, ...
            ...
            for model_index = 1:num_models
                ...
                % ===================================
                figure
                % qq_plot_err_true_est(err_U_trials - mean(err_U_trials, 'omitnan'), err_est_indiv, err_est_comb, ...
                %                     individual_method_array, resampling_method_names_plot_short, ...
                %                     max_error_threshold, 2, colors, symbols, user_screen_resolution);
                qq_plot_err_true_est(err_U_trials, {err_est_indiv{:, model_index}}, {err_est_comb{:, model_index}}, ...
                                    individual_method_array, resampling_method_names_plot_short, ...
                                    max_error_threshold, 2, colors, symbols, user_screen_resolution);
                drawnow();
