...
%         plot_unc_ratio_with_particle_removal_single(unc_ratio{resampling_method_index_plot}, unc_ratio_fit{resampling_method_index_plot}, trial_index, ppr, ...
%                                             individual_method_array, resampling_method_names_plot_short, ...
%                                             ['Uncertainty Ratio ' upper(metric_name)], user_screen_resolution);
...
Y = discretize(abs(U_shear_valid_consolidated), bins_grad);
Y_ref = discretize(abs(U_shear_ref_valid_consolidated), bins_grad);
...
for bin_index = 1:num_bins_grad
    ...
    unc_comb_rms_binned_ref(bin_index, :) = nanrms(unc_comb_binned_ref, 1);
end
...
if save_figures
    save_figure_to_png_svg_fig(figure_save_directory, 'shear', [1, 0, 0]);
end

figure
subplot(1, 2, 1)
% bar(bins_grad, num_indices/(2 * num_valid_trials_consolidated) * 100)
bar(bins_grad, num_indices)
box off
xlim([bins_grad(1) bins_grad(end)])
...
if save_figures
    save_figure_to_png_svg_fig(figure_save_directory, 'shear-bins', [1, 0, 0]);
end
return;
