...
figure_save_directory = fullfile(current_read_directory, 'figures-3cweights');
...
for method_index = 1:num_uncertainty_methods
    for i = 1:4
        ...
        % plot(rms_err(i) * ones(1, 2), [0, y_lim{method_index, i}(2)], 'k--')
        % plot uncertainty rms
        plot(rms_unc_resampled(method_index, i) * ones(1, 2), [0, y_lim{method_index, i}(2)], '--', 'color', colors{i});
