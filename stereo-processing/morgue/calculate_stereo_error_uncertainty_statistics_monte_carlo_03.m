...
weights_all = cell(num_combination_methods, num_individual_methods);
...
for trial_index = 1:num_trials
    ...
    for camera_index = 1:num_cameras
        for method_index = 1:num_individual_methods
            unc_planar_individual_all{camera_index, method_index}(1, trial_index) = real(unc_planar_individual{camera_index, trial_index}.([lower(individual_method_array{method_index}) 'x']));
            unc_planar_individual_all{camera_index, method_index}(2, trial_index) = real(unc_planar_individual{camera_index, trial_index}.([lower(individual_method_array{method_index}) 'y']));            
        end
    end
    ...
    for camera_index = 1:num_cameras
        for method_index = 1:num_individual_methods
            unc_planar_resampled_all{camera_index, method_index}(1, start_index:stop_index) = real(unc_planar_resampling{camera_index, trial_index}.([lower(individual_method_array{method_index}) 'x']));
            unc_planar_resampled_all{camera_index, method_index}(2, start_index:stop_index) = real(unc_planar_resampling{camera_index, trial_index}.([lower(individual_method_array{method_index}) 'y']));
        end
    end
...
% valid_indices_individual = cell(num_combination_methods, num_components);
for camera_index = 1:num_cameras
    for method_index = 1:num_individual_methods
...
unc_stereo_combined_all_valid = cell(num_combination_methods, num_components+1);
valid_indices_combined = cell(num_combination_methods, num_components);
for method_index = 1:num_combination_methods
...
pdf_unc_combined = cell(num_combination_methods, num_components+1);
cdf_unc_combined = cell(num_combination_methods, num_components+1);
for method_index = 1:num_combination_methods
    for component_index = 1:(num_components+1)
...
% uncertainties - combined
rms_unc_combined = nans(num_combination_methods, num_components+1);
for method_index = 1:num_combination_methods
    for component_index = 1:(num_components+1)
...
pdf_weights = cell(num_combination_methods, num_individual_methods);
weights_all_valid = cell(num_combination_methods, num_individual_methods);
