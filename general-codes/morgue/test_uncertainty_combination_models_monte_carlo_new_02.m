...
for window_size_index = 1:num_window_size
    ...
    for dataset_index = 1:numel(dataset_name_array)
        ...
        for snapshot_index = 1:numel(results_all{window_size_index, dataset_index})
            ...
            % IM
            results_all{window_size_index, dataset_index}(snapshot_index).w_glob.im.x = 1/var(results.uncertainty2D.Uimx(r, c), [], 'all', 'omitnan');
            results_all{window_size_index, dataset_index}(snapshot_index).w_glob.im.y = 1/var(results.uncertainty2D.Uimy(r, c), [], 'all', 'omitnan');
            
            % MC
            results_all{window_size_index, dataset_index}(snapshot_index).w_glob.mc.x = 1/var(results.uncertainty2D.MCx(r, c), [], 'all', 'omitnan');
            results_all{window_size_index, dataset_index}(snapshot_index).w_glob.mc.y = 1/var(results.uncertainty2D.MCy(r, c), [], 'all', 'omitnan');
            
            % CS
            results_all{window_size_index, dataset_index}(snapshot_index).w_glob.cs.x = 1/var(real(results.uncertainty2D.Ucsx(r, c)), [], 'all', 'omitnan');
            results_all{window_size_index, dataset_index}(snapshot_index).w_glob.cs.y = 1/var(real(results.uncertainty2D.Ucsy(r, c)), [], 'all', 'omitnan');
        end 
    end
end
...
% uncertainties
unc.im.x = nans(1, num_trials);
unc.im.y = nans(1, num_trials);
unc.mc.x = nans(1, num_trials);
unc.mc.y = nans(1, num_trials);
unc.cs.x = nans(1, num_trials);
unc.cs.y = nans(1, num_trials);

unc.unwt.x = nans(1, num_trials);
unc.unwt.y = nans(1, num_trials);
unc.glob.x = nans(1, num_trials);
unc.glob.y = nans(1, num_trials);
unc.loc.x = nans(1, num_trials);
unc.loc.y= nans(1, num_trials);

% global weights
w_glob.im.x = nans(1, num_trials);
w_glob.im.y = nans(1, num_trials);
w_glob.mc.x = nans(1, num_trials);
w_glob.mc.y = nans(1, num_trials);
w_glob.cs.x = nans(1, num_trials);
w_glob.cs.y = nans(1, num_trials);

% local weights
w_loc.im_x = nans(1, num_trials);
w_loc.im_y = nans(1, num_trials);
w_loc.mc_x = nans(1, num_trials);
w_loc.mc_y = nans(1, num_trials);
w_loc.cs_x = nans(1, num_trials);
w_loc.cs_y = nans(1, num_trials);

% uncertainty from resampling
unc_avg = cell(1, num_trials);
unc_std = cell(1, num_trials);
...
for trial_index = 1:num_trials
    ...
    % IM
    w_glob.im.x(trial_index) = results.w_glob.im.x;
    w_glob.im.y(trial_index) = results.w_glob.im.y;
    
    % MC
    w_glob.mc.x(trial_index) = results.w_glob.mc.x;
    w_glob.mc.y(trial_index) = results.w_glob.mc.y;

    % CS
    w_glob.cs.x(trial_index) = results.w_glob.cs.x;
    w_glob.cs.y(trial_index) = results.w_glob.cs.y;
    ...
    unc.im.x(trial_index) = results.uncertainty2D.Uimx(grid_point_row_index, grid_point_col_index);
    unc.im.y(trial_index) = results.uncertainty2D.Uimy(grid_point_row_index, grid_point_col_index);

    unc.mc.x(trial_index) = results.uncertainty2D.MCx(grid_point_row_index, grid_point_col_index);
    unc.mc.y(trial_index) = results.uncertainty2D.MCy(grid_point_row_index, grid_point_col_index);

    unc.cs.x(trial_index) = real(results.uncertainty2D.Ucsx(grid_point_row_index, grid_point_col_index));
    unc.cs.y(trial_index) = real(results.uncertainty2D.Ucsy(grid_point_row_index, grid_point_col_index));

    %% calculate combined uncertainty for that point for unweighted average
    
    unc.unwt.x(trial_index) = 1/3 .* (unc.im.x(trial_index) + unc.mc.x(trial_index) + unc.cs.x(trial_index));
    unc.unwt.y(trial_index) = 1/3 .* (unc.im.y(trial_index) + unc.mc.y(trial_index) + unc.cs.y(trial_index));
    
    %% calculate combined uncertainty for that point for global weighted average
    
    unc.glob.x(trial_index) = 1./(w_glob.im.x(trial_index) + w_glob.mc.x(trial_index) + w_glob.cs.x(trial_index)) .* (w_glob.im.x(trial_index) .* unc.im.x(trial_index) + w_glob.mc.x(trial_index) .* unc.mc.x(trial_index) + w_glob.cs.x(trial_index) .* unc.cs.x(trial_index));
    unc.glob.y(trial_index) = 1./(w_glob.im.y(trial_index) + w_glob.mc.y(trial_index) + w_glob.cs.y(trial_index)) .* (w_glob.im.y(trial_index) .* unc.im.y(trial_index) + w_glob.mc.y(trial_index) .* unc.mc.y(trial_index) + w_glob.cs.y(trial_index) .* unc.cs.y(trial_index));
    ...
    [unc_avg{trial_index}, unc_std{trial_index}, unc_trials{trial_index}] = calculate_variance_resampling(im1_sub, im2_sub, window_size_x/2, window_size_y/2, 0, 0, x1, y1, x2, y2, d_p*ones(size(x1)), num_particles_remove, num_resampling_trials, uncertainty_flags, intensity_threshold, [window_size_x, window_size_y], [window_resolution_x, window_resolution_y]);
...
%     end
    ...
    w_loc.im_x(trial_index) = 1/unc_std{trial_index}.imx^2/(1/unc_std{trial_index}.imx^2 + 1/unc_std{trial_index}.mcx^2 + 1/unc_std{trial_index}.csx^2);
    w_loc.im_y(trial_index) = 1/unc_std{trial_index}.imy^2/(1/unc_std{trial_index}.imy^2 + 1/unc_std{trial_index}.mcy^2 + 1/unc_std{trial_index}.csy^2);

    w_loc.mc_x(trial_index) = 1/unc_std{trial_index}.mcx^2/(1/unc_std{trial_index}.imx^2 + 1/unc_std{trial_index}.mcx^2 + 1/unc_std{trial_index}.csx^2);
    w_loc.mc_y(trial_index) = 1/unc_std{trial_index}.mcy^2/(1/unc_std{trial_index}.imy^2 + 1/unc_std{trial_index}.mcy^2 + 1/unc_std{trial_index}.csy^2);

    w_loc.cs_x(trial_index) = 1 - (w_loc.im_x(trial_index) + w_loc.mc_x(trial_index));
    w_loc.cs_y(trial_index) = 1 - (w_loc.im_y(trial_index) + w_loc.mc_y(trial_index));

    %% calculate locally weighted uncertainty    
    
    unc.loc.x(trial_index) = 1./(w_loc.im_x(trial_index) + w_loc.mc_x(trial_index) + w_loc.cs_x(trial_index)) .* (w_loc.im_x(trial_index) .* unc.im.x(trial_index) + w_loc.mc_x(trial_index) .* unc.mc.x(trial_index) + w_loc.cs_x(trial_index) .* unc.cs.x(trial_index));
    unc.loc.y(trial_index) = 1./(w_loc.im_y(trial_index) + w_loc.mc_y(trial_index) + w_loc.cs_y(trial_index)) .* (w_loc.im_y(trial_index) .* unc.im.y(trial_index) + w_loc.mc_y(trial_index) .* unc.mc.y(trial_index) + w_loc.cs_y(trial_index) .* unc.cs.y(trial_index));
...
filename = fullfile(current_write_directory, 'calculations_new.mat');
save(filename, 'unc', 'w_glob', 'unc_avg', 'unc_std', 'unc_trials', 'err_U', 'err_V');
...
sigma_im_all_valid = nan_invalid_measurements([unc.im.x(:); unc.im.y(:)], min_error_threshold, max_error_threshold);
% remove invalid measurements for MC uncertainty
sigma_mc_all_valid = nan_invalid_measurements([unc.mc.x(:); unc.mc.y(:)], min_error_threshold, max_error_threshold);
% remove invalid measurements for CS uncertainty
sigma_cs_all_valid = nan_invalid_measurements([unc.cs.x(:); unc.cs.y(:)], min_error_threshold, max_error_threshold);
% remove invalid measurements for the unweighted uncertainty
sigma_unwt_all_valid = nan_invalid_measurements([unc.unwt.x(:); unc.unwt.y(:)], min_error_threshold, max_error_threshold);
% remove invalid measurements for the globally weighted uncertainty
sigma_glob_all_valid = nan_invalid_measurements([unc.glob.x(:); unc.glob.y(:)], min_error_threshold, max_error_threshold);
% remove invalid measurements for the locally weighted uncertainty
sigma_loc_all_valid = nan_invalid_measurements([unc.loc.x(:); unc.loc.y(:)], min_error_threshold, max_error_threshold);
...
w_im_all_glob = [w_glob.im.x./(w_glob.im.x + w_glob.mc.x + w_glob.cs.x), ...
    w_glob.im.y./(w_glob.im.y + w_glob.mc.y + w_glob.cs.y)];
w_mc_all_glob = [w_glob.mc.x./(w_glob.im.x + w_glob.mc.x + w_glob.cs.x), ...
    w_glob.mc.y./(w_glob.im.y + w_glob.mc.y + w_glob.cs.y)];
w_cs_all_glob = [w_glob.cs.x./(w_glob.im.x + w_glob.mc.x + w_glob.cs.x), ...
    w_glob.cs.y./(w_glob.im.y + w_glob.mc.y + w_glob.cs.y)];
...
% calculate normalized weights for all methods, combining x and y weights
w_im_all_loc = [w_loc.im_x./(w_loc.im_x + w_loc.mc_x + w_loc.cs_x), ...
    w_loc.im_y./(w_loc.im_y + w_loc.mc_y + w_loc.cs_y)];
w_mc_all_loc = [w_loc.mc_x./(w_loc.im_x + w_loc.mc_x + w_loc.cs_x), ...
    w_loc.mc_y./(w_loc.im_y + w_loc.mc_y + w_loc.cs_y)];
w_cs_all_loc = [w_loc.cs_x./(w_loc.im_x + w_loc.mc_x + w_loc.cs_x), ...
    w_loc.cs_y./(w_loc.im_y + w_loc.mc_y + w_loc.cs_y)];
