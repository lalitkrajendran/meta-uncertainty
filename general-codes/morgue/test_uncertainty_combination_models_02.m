...
for dataset_index = [1] %1:numel(dataset_name_array)
    ...
    for uncertainty_combination_method_index = 1:numel(uncertainty_combination_method_array)
        ...
        uncertainty_combination_method = 'local-weight-var';
...
%                         end
                        ...
                        zone1 = im1( max([1 ymin]):min([image_height ymax]),max([1 xmin]):min([image_width xmax]));
                        zone2 = im2( max([1 ymin]):min([image_height ymax]),max([1 xmin]):min([image_width xmax]));
...
%                         im1_sub = im1d(ymin:ymax, xmin:xmax);
%                         im2_sub = im2d(ymin:ymax, xmin:xmax);
                        ...
                        %% remove particles and perform bootstrapping calc
                        ...
                        [sigma_avg, sigma_std] = calculate_variance_boostrapping(im1_sub, im2_sub, X(r,c), Y(r,c), U(r,c), V(r,c), x1, y1, x2, y2, d_p*ones(size(x1)), num_particles_remove, num_bootstrap_trials, uncertainty_flags, intensity_threshold);
