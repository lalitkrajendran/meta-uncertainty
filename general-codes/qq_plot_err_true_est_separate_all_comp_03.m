function lgd = qq_plot_err_true_est_separate_all_comp_03(err_all_valid, err_est_valid_individual, err_est_valid_combined, ...
    ...
    for component_index = 1:num_components
        ...
        % set(xl, 'VerticalAlignment', 'baseline');
        % xl.Position(1) = max_error_threshold(component_index);
        xl.Position(2) = -0.55 * max_error_threshold(component_index); %-0.15;
    ...
    lgd = legend(l_all, {individual_method_array{:}, combination_method_array{combination_method_plot_index_array}}, 'location', 'northoutside', 'orientation', 'horizontal'); %, 'NumColumns', 2);
    % adjust figure size
    set(gcf, 'resize', 'off');
    drawnow();
    ...
    set(gcf, 'units', 'inches', 'Position', [23   311   750   350]/user_screen_resolution);
    drawnow();
    lgd.Position = [0.4000 0.9 0.2077 0.0412];
