function plot_pdf_err_true_est(bins, pdf_err, pdf_err_est_individual, pdf_err_est_combined, max_error_threshold, uncertainty_method_array, combination_method_names, ...
    ...
    for uncertainty_method_index = 1:num_uncertainty_methods    
        plot(bins(1:end-1), pdf_err_est_individual(uncertainty_method_index, :), line_symbols{uncertainty_method_index}, 'color', colors(1, :))
        hold on
    end
    plot(bins(1:end-1), pdf_err, 'k')
    ...
    for combination_method_index = 1:num_combination_methods  
        plot(bins(1:end-1), pdf_err_est_combined(combination_method_index, :), line_symbols{combination_method_index}, 'color', colors(2, :))
        hold on
    end
    plot(bins(1:end-1), pdf_err, 'k')
    xlim([0 max_error_threshold])
